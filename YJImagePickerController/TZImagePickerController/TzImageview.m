...
@interface TzImageview ()<TZImagePickerControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate,UINavigationControllerDelegate,JJPhotoDelegate>

{
  ...
  BOOL  allowPickingGifSwitch;///< 允许选择GIF图片
  BOOL  showSheetSwitch;///< 显示一个sheet,把拍照按钮放在外面
  BOOL  showTakePhotoBtnSwitch;///< 在内部显示拍照按钮
  BOOL  sortAscendingSwitch;///< 照片排列按修改时间升序
  BOOL  allowPickingVideoSwitch;///< 允许选择视频
    BOOL  allowPickingImageSwitch;///< 允许选择图片
  BOOL  allowPickingOriginalPhotoSwitch;///< 允许选择原图
    NSString*  maxCountTF;///< 照片最大可选张数，设置为1即为单选模式
    
    NSString*  columnNumberTF; // 每行展示照片
    BOOL  allowCropSwitch;// 单选模式允许裁剪
    BOOL needCircleCropSwitch;//使用圆形剪裁框
    BOOL allowPickingMuitlpleVideoSwitch; // 允许多选视频／GIF图片//
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
       ...
       // [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(xiangceClick) name:@"dakaixiangce" object:nil];
        allowPickingGifSwitch=NO;
        showSheetSwitch=YES;
        showTakePhotoBtnSwitch=NO;
        allowPickingVideoSwitch=NO;
        allowPickingImageSwitch=YES;
        allowPickingOriginalPhotoSwitch=YES;
        sortAscendingSwitch=YES;
        maxCountTF=@"9";
        columnNumberTF=@"5";
        allowCropSwitch=NO;
        needCircleCropSwitch=NO;
        allowPickingMuitlpleVideoSwitch=NO;
    }
    return self;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == _selectedPhotos.count) {
        cell.imageView.image = [UIImage imageNamed:@"tianjia_5.png"];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == _selectedPhotos.count) {
        ...
        if ([[asset valueForKey:@"filename"] tz_containsString:@"GIF"] && allowPickingGifSwitch && !allowPickingMuitlpleVideoSwitch) {
...
//            imagePickerVc.maxImagesCount = maxCountTF.integerValue;
...
//            __weak typeof (imagePickerVc)PickerVc=imagePickerVc;
//            [imagePickerVc setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
//                _selectedPhotos = [NSMutableArray arrayWithArray:photos];
//                _selectedAssets = [NSMutableArray arrayWithArray:assets];
//                PickerVc.isSelectOriginalPhoto =weakself.isSelectOriginalPhoto;
//                [_collectionView reloadData];
//                // _collectionView.contentSize = CGSizeMake(0, ((_selectedPhotos.count + 2) / 3 ) * (_margin + _itemWH));
//            }];
//            [[self currentViewController] presentViewController:imagePickerVc animated:YES completion:nil];
            TZTestCell *cell = (TZTestCell*)[collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:indexPath.row inSection:0]];
...
- (void)pushTZImagePickerController {
    if (maxCountTF.integerValue <= 0) {
        return;
    }
    TZImagePickerController *imagePickerVc = [[TZImagePickerController alloc] initWithMaxImagesCount:maxCountTF.integerValue columnNumber:columnNumberTF.integerValue delegate:self pushPhotoPickerVc:YES];
    // imagePickerVc.navigationBar.translucent = NO;
    
#pragma mark - 五类个性化设置，这些参数都可以不传，此时会走默认设置
    imagePickerVc.isSelectOriginalPhoto = _isSelectOriginalPhoto;
    
    if (maxCountTF.integerValue > 1) {
        // 1.设置目前已经选中的图片数组
